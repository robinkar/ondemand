...
class RcloneUtil
  ...
  class << self
    ...
    def mkdir(remote, path)
      ...
      # TODO: detect when Rclone remote doesn't support empty dirs (e.g. S3) and
      # raise a suitable error message.
      if !s.success?
        raise "Error creating directory: #{e}"
      end
    end
